...
@section('title', 'Departments - NoteVault')
...
<div class="max-w-6xl mx-auto">
    <div class="mb-8">
        ...
        <p class="text-gray-600 dark:text-gray-400 mt-1">Browse departments and their courses</p>
    </div>
                ...
                <a href="{{ route('departments.show', $department) }}"
                    ...
                    <div class="flex items-center gap-3 mb-3">
                        <div class="w-10 h-10 rounded-lg bg-blue-50 dark:bg-blue-900/20 flex items-center justify-center flex-shrink-0">
                            <i class="bx bx-buildings text-lg text-blue-600 dark:text-blue-400"></i>
                        </div>
                        <h3 class="font-semibold text-gray-900 dark:text-white">{{ $department->name }}</h3>
                    </div>
                    @if ($department->code)
                        <p class="text-xs text-gray-500 dark:text-gray-400 mb-2">{{ $department->code }}</p>
                    @endif
                    <p class="text-sm text-gray-600 dark:text-gray-400">{{ $department->courses_count }} {{ Str::plural('course', $department->courses_count) }}</p>
                </a>
            @endforeach
        </div>
    ...
    @else
        <div class="bg-white dark:bg-gray-800 rounded-xl shadow-sm border border-gray-200 dark:border-gray-700 p-12 text-center">
            <i class="bx bx-buildings text-6xl text-gray-300 dark:text-gray-600 mb-4"></i>
